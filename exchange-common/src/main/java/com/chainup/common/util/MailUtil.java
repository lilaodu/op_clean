...
public class MailUtil {
	...
	public MailUtil() {
		try {
			from = PropertyUtil.getProperties("email.sendmail.from");
			username = PropertyUtil.getProperties("email.sendmail.username");
			password = PropertyUtil.getProperties("email.sendmail.password");
			host = PropertyUtil.getProperties("email.smtp");
			//port = PropertyUtil.getProperties("mye.smtp.port");
			//auth = PropertyUtil.getProperties("mye.smtp.auth");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * 返回类型：
	 */
	public MailUtil(String to, String from, String smtpServer, String username, String password, String subject, String content) {
		//this.to = to;
		this.from = from;
...
//      Session session = Session.getDefaultInstance(props, null);  
        ...
        try {  
           ...
           // msg.setHeader("X-Mailer", "MyE Mailer");
...
//          }  
	...
	public static void main(String[] args) {
		//float rate = 0.04561f;
		//System.out.println((rate*100f-(int)(rate*10f)*10f+20f)/100f);
		MailUtil sendmail = new MailUtil();
		
		sendmail.sendMail("egonzou@qq.com", "测试邮件", "这是一封测试邮件正文");
