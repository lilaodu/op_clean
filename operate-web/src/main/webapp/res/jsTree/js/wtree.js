...
function dTree(objName,iconPath) {
	this.config = {
		...
		inOrder					: false,
		useRadio: false,
		check:false		//to control whether checkbox is showed 
	};
	//in order to make client can modify path of icons;
	...
	//to cache checkbox object and improve speed of set checked status
...
dTree.prototype.getCheckedNodes=function(){
	...
	for(var n=0; n<this.aNodes.length; n++){
		...
		if(cko==null || cko.checked == undefined){
		}
		else if(cko.checked==true){
...
dTree.prototype.ra=function(id){
	if (this.cbCollection[id])return this.cbCollection[id];
	for(var n=0; n<this.aNodes.length; n++){
		if(this.aNodes[n].id==id){
...
dTree.prototype.getSelectedNode=function(){
	...
	var cko;//checkobject
	for(var n=0; n<this.aNodes.length; n++){
		cko = this.ra(this.aNodes[n].id);
		if(cko.checked==true){
			res=this.aNodes[n].id;
			break;
		}
	}
	return res;
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.check==true){
		str+= '<input type="checkbox" class="cx" id="c'+  this.obj + nodeId + '" name=' + node.id + ' onclick="javascript:'+this.obj+'.cc('+nodeId+')"/>';
		//alert(str);
	}
